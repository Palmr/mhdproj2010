...
try:
  ...
  def Ipl2NumPy(input):
      ...
      if input.nChannels == 1:
          ...
          rgb = cv.CreateImage((input.width, input.height), input.depth, 3)
          cv.CvtColor(input, rgb, cv.CV_BGR2RGB)
          array_1d = numpy.fromstring(rgb.tostring(), dtype=ipl2dtype[key])
          return numpy.reshape(array_1d, (input.height, input.width, input.nChannels))
